...
public class JdbcUserRepository implements UserRepository {
    ...
    public void addFriend(User user, User friend) {
        final String INSERT_QUERY = "INSERT INTO USERFRIENDS (USER_ID,FRIEND_ID) VALUES(:u_id,:f_id)"; //+dB
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
    ...
    private boolean checkExistUSERFrends(Long userId, Long friendId) {
        boolean ret = true;
        final String SELECT_QUERY = "SELECT count(*) FROM USERFRIENDS WHERE USER_ID = :u_id AND FRIEND_ID = :f_id"; //+dB
    ...
    public void deleteFriend(User user, User friend) {
        final String DELETE_QUERY = "DELETE FROM USERFRIENDS WHERE USER_ID = :u_id AND FRIEND_ID = :f_id"; //+ dB
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("u_id", user.getId());
        mapSqlParameterSource.addValue("f_id", friend.getId());
    ...
    public List<User> getFriends(User user) {
        final String SELECT_QUERY = "SELECT * FROM USERS AS u INNER JOIN USERFRIENDS AS uf  ON uf.FRIEND_ID = u.USER_ID WHERE uf.USER_ID = :u_id "; //+dB
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("u_id", user.getId());
    ...
    public List<User> getMutualFriends(User user, User otherUser) {
        final String SELECT_QUERY = "SELECT * FROM USERS " +
                " JOIN USERFRIENDS AS USFirst ON USFirst.FRIEND_ID  = USERS.USER_ID" +
                " JOIN USERFRIENDS AS USSecond" +
